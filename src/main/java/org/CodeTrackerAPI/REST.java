...
public class REST {
        ...
        Handlers
          ...
          .get(
            ...
            exchange -> {
              ...
              Integer endLineNumber = 0;
              String latestCommitHash;

              try {
                parentMethod = params.get("parentMethod").getFirst();
                parentMethodLine =
                  Integer.parseInt(params.get("parentMethodLine").getFirst());
                endLineNumber =
                  Integer.parseInt(params.get("endLineNumber").getFirst());
              } catch (Exception e) {}
  ...
  private static String CTBlock(
    ...
    String blockName,
    Integer methodLineNumber,
    Integer blockStartLineNumber,
    Integer blockEndLineNumber
  ) {
    ArrayList<CTHBlock> changeLog = new ArrayList<>();
    try {

      LocationInfo.CodeElementType blockType = null;

      if (blockName.equals("for")) {
        blockType = LocationInfo.CodeElementType.FOR_STATEMENT;
      } else if (blockName.equals("try")) {
        blockType = LocationInfo.CodeElementType.TRY_STATEMENT;
      } else if (blockName.equals("if")) {
        blockType = LocationInfo.CodeElementType.IF_STATEMENT;
      }
      ...
      BlockTracker blockTracker = CodeTracker
        ...
        .blockStartLineNumber(blockStartLineNumber)
        .blockEndLineNumber(blockEndLineNumber)
        .build();
